...
public interface VotesMapper {
    ...
    @Insert("insert into votes(task_uuid, user_uuid, vote, create_dtm) " +
            "values(#{taskUuid}, #{votingUserUuid}, #{voteValue}, #{createDtm})" +
    ...
    void createVote(@Param("taskUuid") UUID taskUuid,
                    ...
                    @Param("voteValue") int vote,
                    @Param("createDtm") Instant createDtm);

    @ConstructorArgs(value = {
            @Arg(column = "user_name", javaType = String.class),
            @Arg(column = "vote", javaType = int.class)
    })
    @Select("select u.name as user_name, v.vote " +
            "from votes v " +
